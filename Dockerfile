...
COPY --from=base packages/os/build-steps /build-steps
...
COPY --from=base packages/os/overlay-common /
COPY --from=base "packages/os/overlay-${TARGETARCH}" /
...
# Launcher and fix
COPY source/modules/apps/apps.ts    /opt/umbreld/source/modules/apps/apps.ts
